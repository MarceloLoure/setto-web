...
export default function LandingClient({ initialData }: LandingClientProps) {
  ...
  return (
    <Box sx={{ overflowX: 'hidden' }}>
        ...
        <Grid container spacing={6} alignItems="center">
          <Grid size={{ xs: 12, md: 7 }} sx={{ justifySelf: { xs: 'center', md: 'start' }, textAlign: { xs: 'center', md: 'start' } }}>
            ...
            <Typography variant="h6" color="text.secondary" fontWeight={400} sx={{ mb: 4, maxWidth: 520 }}>
              O Setto cuida da grade de horários, cobra o atleta na hora da reserva e libera o financeiro da sua arena de qualquer planilha.
            </Typography>
      ...
      {/* Prova social */}
      {initialData && initialData.stats?.activeArenas > 0 && (
        <>
          <Container maxWidth="lg"><CourtLine /></Container>
          <Container maxWidth="lg" sx={{ py: 5 }}>
            <Grid container spacing={4} justifyContent="center" alignItems="center">
              {[
                { value: initialData.stats.activeArenas, label: 'arenas ativas na plataforma' },
                { value: initialData.stats.activeCourts, label: 'quadras gerenciadas' },
                { value: initialData.stats.totalBookings, label: 'reservas realizadas' },
              ].map((stat) => (
      ...
      <Container maxWidth="lg" sx={{ py: { xs: 6, md: 8 }, textAlign: 'center' }} id="planos">
        ...
        {!initialData || !initialData.plans || initialData.plans.length === 0 ? (
          <Typography color="text.secondary">
            Nenhum plano disponível no momento. Fale com a gente para uma proposta sob medida.
          </Typography>
        ) : (
            <Grid container spacing={3} mt={2} justifyContent="center">
                {initialData.plans.map((plan: any, index: number) => {
                    const isHighlighted = index === Math.floor((initialData.plans.length - 1) / 2) && initialData.plans.length > 1;
                    
                    // Extrai a cor diretamente em uma variável para evitar parsing dinâmico de AST no sx
                    const borderColor = isHighlighted ? 'primary.main' : 'divider';
        ...
        <Grid container spacing={4} justifyContent="space-between">
          <Grid size={{ xs: 12, sm: 5 }} textAlign={'center'} justifyItems={'center'}>
            ...
            <Typography variant="caption" color="text.secondary" display="block">
                © 2026 Setto. Todos os direitos reservados.
            </Typography>
          </Grid>
